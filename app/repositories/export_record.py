...
class ExportRecordRepository(BaseRepository[ExportRecord]):
    ...
    def list_with_filters(
        ...
        *,
        ...
        stmt = self._apply_filters(stmt, client_id, status, collaborator_id, search, vessel, date_from, date_to, ets_from, ets_to, completed_from, completed_to, billing_completed)
    ...
    def _apply_filters(
        ...
        if vessel is not None:
            stmt = stmt.where(ExportRecord.vessel.ilike(f"%{vessel}%"))
        ...
        if completed_from is not None:
            stmt = stmt.where(ExportRecord.completed_at >= completed_from)
        if completed_to is not None:
            stmt = stmt.where(ExportRecord.completed_at <= completed_to)
        if billing_completed is not None:
            stmt = stmt.where(ExportRecord.billing_completed == billing_completed)
        return stmt
