...
export default function ExportList() {
  ...
  const [collaboratorId, setCollaboratorId] = useState(
    () => sessionStorage.getItem('grubba_collaborator_export') ?? ''
  )
  ...
  useEffect(() => {
    sessionStorage.setItem('grubba_collaborator_export', collaboratorId)
  }, [collaboratorId])
    ...
    <div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-slate-200 space-y-2">
          ...
          {/* Row 2: date range + period shortcuts + ETS range */}
        ...
        >
          {ALL_STATUS_OPTIONS.map((opt) => (
            <button
              ...
              onClick={() => handleStatusChange(editingStatusId, opt.value as ExportStatus)}
            >
              {opt.label}
            </button>
          ))}
